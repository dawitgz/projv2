...
@app.route('/catalog/')
def showCatalog():
    ''' Default page, renders the list of catalog items '''
    user = None
...
def gconnect():
    ''' Main function used for Google Authentication '''
...
def gdisconnect():
    ''' Function used to log out a Google user that is already signed in '''
...
def getUserID(email):
    ''' Given an email, returns a user ID or None '''
    try:
...
def addItem():
    ''' Adds a new item, using drop-down to identify the category '''
    if 'username' not in login_session:
        return redirect('/login')
...
def showJsonCategoryItems():
    ''' Returns a json list of category items '''
...
def showItem(category_name, item_name):
    ...
    if (('username' not in login_session) or
        ...
        return render_template('showItem.html', item=item, owner=owner, creator=creator)
